	...
	var methods = {
		...
		imageWidget: function(){
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					if(!f.type.match('image.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ 
		...
		init : function( options )
		{
		    ...
		    $(document.body).mousedown(function(event) {
		        ...
		        if (!target.parents().andSelf().is('#specialchar') && (target.closest('a').html()!='<i class="fa fa-asterisk"></i>')) { 
		        ...
		        if (!target.parents().andSelf().is('#paletteCntr') && (target.closest('a').html()!='<i class="fa fa-font"></i>')) { 
